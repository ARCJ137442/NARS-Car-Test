...
class Constants:
    ...
    #测试文件位置
    # SAVE_URL = "D:/test.xlsx"
    # SHEET_NAME = 'test'
    #结果保存位置
    SAVE_URL = "result_1_1.xlsx"
    #十次重复实验
    SHEET_NAME = 'No.1'
    # SHEET_NAME = 'No.2'
    # SHEET_NAME = 'No.3'
    ...
    # SHEET_NAME = 'No.15'#n8
    # SHEET_NAME = 'No.16'#n9
    # SHEET_NAME = 'No.17'#n10
    # 12.5
    # SHEET_NAME = 'No.18'#n11
    # SHEET_NAME = 'No.19'#n12
    # SHEET_NAME = 'No.20'#n13
    # SHEET_NAME = 'No.21'#n14
    # SHEET_NAME = 'No.22'#n15
    # SHEET_NAME = 'No.23'#n16
    # SHEET_NAME = 'No.24'#n17
    # SHEET_NAME = 'No.25'#n18
    # SHEET_NAME = 'No.26'#n19
    # SHEET_NAME = 'No.27'#n20

    # SHEET_NAME = 'No.28'#n21
    # SHEET_NAME = 'No.29'#n22
    # SHEET_NAME = 'No.30'#n23
    # SHEET_NAME = 'No.31'#n24
    # SHEET_NAME = 'No.32'#n25
    # SHEET_NAME = 'No.33'#n26
    # SHEET_NAME = 'No.34'#n27
    # SHEET_NAME = 'No.35'#n28
    # SHEET_NAME = 'No.36'#n29
    # SHEET_NAME = 'No.37'#n30
...
class Game:
        ...
        for line in iter(out.readline, b'\n'):  # get operations
            ...
            if line[0:3] == 'EXE':
                ...
                if operation == '^left':
                    ...
                    self.condition_judge('left','C')
                    self.move_left('C')
                    Constants.NARS_OP_TIMES += 1
                    Constants.OP_SIGNAL = False
                if operation == '^right':
                    Constants.OP_SIGNAL = True
                    self.condition_judge('right','C')
                    self.move_right('C')
                    Constants.NARS_OP_TIMES += 1
                    Constants.OP_SIGNAL = False
        out.close()
    ...
    def babble(self):
        ...
        #randlist = [2,2,1,2,2,1,2,2,1,2]
        randlist = [2, 2, 2, 2, 2, 2, 2]
        #rand_int = random.randint(1,2)
        rand_int = randlist[Constants.BABBLE_TIMES - 1]
        if rand_int == 1:
            self.condition_judge('left','B')
            self.move_left('B')
            #在发生操作前就判断这次操作的后果是撞车、不撞车或安全，因为位移发生后判断出随之发生错误
        if rand_int == 2:
            self.condition_judge('right','B')
            self.move_right('B')
        Constants.TRAIN_SIGNAL = False

    # 如果接收到移动的指令，先向nars传入移动的信息，之后判断撞车与否，并更新此时的感知信息。
    def move_left(self, who):
        print(self.car.rect.x,who+'动前',int(self.speeding_delta_time_s))
        # 精神运动
        ...
        print(self.car.rect.x, who + '动后', int(self.speeding_delta_time_s))
        self.visdom_data()
        
    
    def move_right(self, who):
        #if who == 'C' and int(self.speeding_delta_time_s) < 20:
        #    print('该返回')
        #    return 0
        print(self.car.rect.x, who + '动前', int(self.speeding_delta_time_s))
        # 运动发生
    ...
    def condition_judge(self,key_value,who):
        #print("------------------------")
        #print("Enter in condition judge")
        #print(key_value)
        #print(self.car.rect.x)
        # print(self.car.rect.x, who, int(self.speeding_delta_time_s))
    ...
    def __set_timer(self):
        UPDATE_NARS_EVENT_TIMER = 1000 # FLAG
    ...
    def __display_text_babble(self):
        ...
        babbling = self.font.render('随机训练次数: %d' % Constants.BABBLE_TIMES, True, Constants.BLACK)
        ...
        self.screen.blit(nars_op,[70,110])
        self.screen.blit(nars_activation,[320,110])
        self.screen.blit(nars_success_count,[70,130])
        self.screen.blit(nars_failure_count,[320,130]) 
        self.screen.blit(nars_sum_count,[70,150])
        self.screen.blit(nars_success_rate,[70,170])

        self.screen.blit(success_count, [70, 200])
        self.screen.blit(failure_count, [320, 200])
        self.screen.blit(sum_count, [70, 220])
        self.screen.blit(success_rate, [320,220])
    ...
    def __display_text_human(self):
        ...
        self.screen.blit(nars_op,[70,110])
        self.screen.blit(nars_success_count,[320,110])
        self.screen.blit(nars_failure_count,[70,130]) 
        self.screen.blit(nars_sum_count,[320,130])
        self.screen.blit(nars_success_rate,[70,150])

        self.screen.blit(success_count, [70, 180])
        self.screen.blit(failure_count, [320, 180])
        self.screen.blit(sum_count, [70, 200])
        self.screen.blit(success_rate, [320,200])
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_SPACE:
                        ...
                        pygame.quit()
                        quit()
        # pause_info = self.font.render('按下空格继续！' ,True, Constants.RED)
        # self.screen.blit(pause_info, [220, 220])
    ...
    def random_babble(self):
        ...
        Constants.Start_time = self.start_time
