...
class Game:
    ...
    def babble(self):
        ...
        if rand_int == 1:
            ...
            self.move_left_babble()
        if rand_int == 2:
            self.send_to_nars('<(*,{SELF}) --> ^right>. :|:')
            self.move_right_babble()
    ...
    def move_left_human(self):
        self.car.move_left()  # 在Sprites里修改控制步长
        self.send_to_nars('<(*,{SELF}) --> ^left>. :|:')

    def move_right_human(self):
        self.car.move_right()
        self.send_to_nars('<(*,{SELF}) --> ^right>. :|:')
        ...
        while True:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    # 键盘操作在这里
                    if event.key == pygame.K_LEFT and self.car.rect.left >= 96:
                        self.move_left_human()
                    elif event.key == pygame.K_RIGHT and self.car.rect.right <= 666:
