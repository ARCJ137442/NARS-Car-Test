...
class Game:
    ...
    def move_left(self):
        ...
        # 应该在给nars传入应小车移动造成的感知信息变化前告知nars发生了运动操作。
    ...
    def __display_text_babble(self):
        ...
        surface_babbling = self.font.render(
            '随机次数: %d' % Constants.BABBLE_TIMES, True, Constants.BLACK)
    ...
    def __display_text_human(self):
        ...
        surface_fps = self.font.render(
            'FPS: %d' % self.clock.get_fps(), True, Constants.BLACK)
        surface_human = self.font.render(
            '键盘操作次数: %d' % Constants.KEY_TIMES, True, Constants.BLACK)
        success_count = self.font.render(
            '成功次数：%d' % Constants.SUCCESS_COUNT, True, Constants.BLACK)
        failure_count = self.font.render(
            '失败次数：%d' % Constants. FAILURE_COUNT, True, Constants.BLACK)
    ...
    def run(self):
        ...
        self.menu.add.button(
            'Random Babble', self.random_babble)  # 根据按钮不同，进入不同方法中
...
if __name__ == '__main__':
    ...
    game = Game()  # 初始化
    game.launch_nars("./opennars.jar")
    # time.sleep(3)
    # 常识输入
    game.send_to_nars('<<{SELF} --> [safe]> ==> <{SELF} --> [good]>>. :|:')
    game.send_to_nars('<{SELF} --> [good] >! :|:')
    game.run()  # 运行游戏
