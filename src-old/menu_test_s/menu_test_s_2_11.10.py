...
class Game:
    ...
    def connect_nars(self, host, port):
        ...
        self.launch_thread()
        # 输入实验所需的常识
        self.send_to_nars('<{SELF} --> [safe] >! :|:') # 初始状态：NARS并不GOOD
        self.send_to_nars('<{lsensor, rsensor} --> {SELF} >. :|:') #  初始状态：NARS目标是让自己GOOD
...
# 如果同时开事件传送和操作后传送，给nars传入的消息太多，但是信息多并不是与其效果成正比。这段代码负责间隔0.2s持续向nars输入感知信息
    ...
    def move_left(self):
        ...
        #self.send_to_nars("<{lsensor} --> {" + str(self.car.rect.x - 64) + "}>. :|:") # 告知NARS现在左侧的位置
        #self.send_to_nars("<{rsensor} --> {" + str(self.car.rect.x) + "}>. :|:") # 告知NARS现在右侧的位置
        if self.car.rect.x - Constants.MOVE_DISTANCE  < Constants.WALL_WIDTH: #将车的位置改变后在临界位置并且将judge = 1 传输给nars
            print(time.strftime("左侧撞车___%H:%M:%S\t", time.localtime()))
            # 物理运动
    ...
    def move_right(self):
        ...
        if self.car.rect.x + Constants.MOVE_DISTANCE > Constants.SCREEN_WIDTH-Constants.WALL_WIDTH-Constants.CAR_WIDTH:
            print(time.strftime("右侧撞车___%H:%M:%S\t", time.localtime()))
